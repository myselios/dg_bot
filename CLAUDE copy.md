...
```markdown
새 기능 개발 시:
[ ] 테스트 유형 판단 (아래 "테스트 파일 위치 강제 규칙" 참조)
[ ] 올바른 폴더에 테스트 파일 생성
[ ] 실패하는 테스트 작성
[ ] 테스트 실행하여 실패 확인
[ ] 최소 코드 작성하여 테스트 통과
...
#### 🔴 테스트 파일 위치 강제 규칙 (반드시 준수)

> **⚠️ tests/ 루트에 테스트 파일을 직접 생성하는 것은 금지됩니다.**

**허용되는 파일 위치:**
```
tests/
├── __init__.py          ✅ 허용 (패키지 초기화)
├── conftest.py          ✅ 허용 (공유 fixture)
├── contracts/           ✅ 테스트 파일 위치
├── scenarios/           ✅ 테스트 파일 위치
├── scheduler/           ✅ 테스트 파일 위치
...
├── e2e/                 ✅ 테스트 파일 위치
├── backend/             ✅ 테스트 파일 위치
└── test_*.py            ❌ 금지! 하위 폴더로 이동 필수
```

**테스트 생성 전 필수 질문:**
1. 돈이 새는 위험인가? → `contracts/`
2. 트레이딩 흐름 시나리오인가? → `scenarios/`
3. 스케줄러/운영 안정성인가? → `scheduler/`
4. 백테스팅 로직인가? → `backtesting/`
5. 순수 도메인/유스케이스/어댑터 단위 테스트인가? → `unit/{layer}/`
6. DB/외부 API 통합 테스트인가? → `integration/`
7. 전체 E2E 흐름인가? → `e2e/`

**unit/ 폴더 세부 구조:**
```
tests/unit/
├── domain/              # 도메인 계층 (entities, value_objects, services)
│   ├── entities/
│   ├── services/
│   └── value_objects/
├── application/         # 유스케이스 계층 (ports, use_cases, services)
│   ├── ports/
│   ├── services/
│   └── use_cases/
├── infrastructure/      # 인프라스트럭처 계층 (adapters, persistence)
│   ├── adapters/
│   └── persistence/
├── presentation/        # 프레젠테이션 계층 (CLI, telegram)
├── pipeline/            # 파이프라인 스테이지
├── config/              # 설정 관련
└── container/           # DI 컨테이너
```
...
```
ARCHITECTURE.md (최상위 - Single Source of Truth)
    ↓ 개요 포함 필수
    ├── BACKTESTING_GUIDE.md (상세 가이드)
    ├── SCHEDULER_GUIDE.md (상세 가이드)
    ├── USER_GUIDE.md (상세 가이드)
    └── 기타 하위 가이드
```

**핵심 원칙**:
- **ARCHITECTURE.md는 모든 기능의 개요를 포함해야 함**
- 하위 가이드에 새 기능이 추가되면 ARCHITECTURE.md의 해당 섹션도 반드시 업데이트
- 하위 가이드는 상세 내용, ARCHITECTURE.md는 아키텍처 수준 개요

#### 🔴 문서 크로스 레퍼런스 강제 규칙

**새 기능 구현 또는 기존 기능 수정 시**:

1. **관련 문서 검색 필수** (구현 전):
   ```bash
   # 예: 백테스팅 관련 변경 시
   grep -r "백테스팅\|backtest\|QuickBacktestFilter" --include="*.md" docs/
   ```

2. **발견된 모든 문서 동기화**:
   - 검색 결과로 나온 모든 .md 파일 확인
   - 각 문서의 관련 섹션 일관성 검증
   - 불일치 발견 시 모두 업데이트

3. **PLAN 문서 완료 조건**:
   - 각 Phase 완료 시 **관련 문서 업데이트 항목 명시**
   - Phase 완료 체크박스에 "[ ] 관련 문서(ARCHITECTURE.md, 가이드) 업데이트" 필수 포함

#### 문서 업데이트 체크리스트

```markdown
코드 변경 후 (순서대로 확인):
[ ] 1. grep으로 관련 키워드 검색하여 모든 참조 문서 식별
[ ] 2. ARCHITECTURE.md 해당 섹션 업데이트 (개요 수준)
[ ] 3. 하위 가이드 문서 상세 업데이트
[ ] 4. 다이어그램(docs/diagrams/) 최신 상태 확인
[ ] 5. CLAUDE.md에 새 컴포넌트 반영 (필요시)
[ ] 6. 문서 간 일관성 검증 (동일 기능이 다르게 설명되지 않는지)
```
...
7. **백테스팅 로직 변경**: ARCHITECTURE.md 백테스팅 섹션 + BACKTESTING_GUIDE.md 동시 업데이트
8. **새 가이드 문서 작성**: ARCHITECTURE.md에 해당 기능 개요 섹션 추가 필수

#### ⚠️ 문서 불일치 방지 규칙

**하위 가이드 작성/수정 시**:
1. 해당 기능이 ARCHITECTURE.md에 있는지 확인
2. 없으면 ARCHITECTURE.md에 개요 섹션 먼저 추가
3. 있으면 ARCHITECTURE.md 내용과 일관성 검증
4. 불일치 발견 시 ARCHITECTURE.md를 기준으로 통일

**예시 - BACKTESTING_GUIDE.md에 2-gate 구조 추가 시**:
```bash
# 1. 관련 문서 검색
grep -r "Research Pass\|Trading Pass\|2-gate\|QuickBacktestFilter" --include="*.md" docs/

# 2. ARCHITECTURE.md 백테스팅 섹션 확인
# 3. 2-gate 구조 개요를 ARCHITECTURE.md에 먼저 추가
# 4. BACKTESTING_GUIDE.md에 상세 내용 작성
```

**⚠️ 하위 가이드에만 새 기능을 추가하고 ARCHITECTURE.md를 업데이트하지 않는 것은 금지됩니다.**
