...
limits=RiskLimits(
    stop_loss_pct=-12.0,         # 손절: -12%
    take_profit_pct=15.0,        # 익절: +15% (R:R 1.25:1)
...
### v4.1.0 (2026-01-02)
- Clean Architecture 파이프라인 마이그레이션 완료
- 모든 파이프라인 스테이지 async/await 전환
- Container 기반 DI 도입 (UseCase 패턴)
- ExecutionStage, AnalysisStage UseCase 마이그레이션
- 918개 테스트 통과
