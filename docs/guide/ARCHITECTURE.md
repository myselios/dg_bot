...
│   │   ├── quick_filter.py    # 🆕 2-Gate 필터 (Research/Trading Pass)
│   │   ├── expectancy_filter.py  # 🆕 기대값 필터 (음수 EV 차단)
...
#### 🆕 2-Gate 필터 아키텍처 (`src/backtesting/quick_filter.py`)

백테스팅 필터는 **2-Gate 구조**로 설계되어 AI 비용을 최적화합니다:

```
┌─────────────────────────────────────────────────────────────────┐
│                    2-Gate Filter Architecture                    │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  [Research Pass] ──느슨한 기준──→ [AI Analysis] ──→ [Trading Pass]│
│       (Gate 1)                                          (Gate 2) │
│                                                                  │
│  • 많은 코인 통과        • AI가 최종 판단      • 엄격한 기준      │
│  • 잠재력 있는 후보      • 규칙으로 못 잡는    • 12개 필터 통과   │
│  • 비용: $0             • 패턴 감지           • + Expectancy 필수 │
│                         • 비용: ~$0.09        • 기대값 양수만 진입 │
└─────────────────────────────────────────────────────────────────┘
```

**CoinSelector 통합** (2026-01-04):
- 4-1단계: AI 분석 후 Trading Pass 검증 (`_apply_trading_pass()`)
- `CoinCandidate.is_ready_for_entry`: Trading Pass 통과 필수
- Expectancy 정보 포함 (`expectancy_R` 필드)

**3가지 임계값 레벨**:

| 레벨 | 용도 | 엄격도 | 적용 시점 |
|------|------|--------|-----------|
| `ResearchPassConfig` | Gate 1 | 느슨함 | AI 분석 전 사전 필터링 |
| `TradingPassConfig` | Gate 2 | 중간 | AI 분석 후 최종 검증 |
| `QuickBacktestConfig` | 현재 운영 | 엄격함 | 단일 필터 사용 시 |

#### ExpectancyFilter (`src/backtesting/expectancy_filter.py`) 🆕

- **역할**: 수학적 기대값 기반 필터 (음수 기대값 전략 차단)
- **핵심 공식**: `Expectancy = (Win% × AvgWin) - (Loss% × AvgLoss) - Costs`
- **비용 모델**: 수수료 0.1% + 슬리피지 0.15% = 0.25% (왕복 0.5%)
- **동적 임계값**: 승률에 따라 요구 손익비 자동 계산
- **Trading Pass 통합** (2026-01-04): `evaluate_trading_pass()`에서 필수 조건으로 검증

```python
# 예: 승률 40%일 때 필요한 최소 손익비
min_ratio = get_min_win_loss_ratio(win_rate=40.0)  # → 1.75

# Trading Pass에서 자동 검증
trading_result = filter.evaluate_trading_pass(metrics)  # Expectancy 포함
```
...
- **2-Gate 메서드**:
  - `evaluate_research_pass()`: 느슨한 기준으로 후보 선별
  - `evaluate_trading_pass()`: 엄격한 기준으로 최종 검증

**12가지 필터 조건** (카테고리별 3단계 임계값):

| 카테고리 | 필터 | Research | Trading | Current | 설명 |
|---------|------|----------|---------|---------|------|
| **수익성** | 수익률 | ≥ 5% | ≥ 10% | ≥ 15% | 2년간 누적 |
| | 승률 | ≥ 30% | ≥ 35% | ≥ 38% | 돌파 전략 특성 |
| | 손익비 | ≥ 1.2 | ≥ 1.5 | ≥ 1.8 | 수수료 고려 |
| **위험조정** | Sharpe | ≥ 0.5 | ≥ 0.8 | ≥ 1.0 | 기관 기준 |
| | Sortino | ≥ 0.6 | ≥ 1.0 | ≥ 1.2 | 하방 리스크 |
| | Calmar | ≥ 0.4 | ≥ 0.6 | ≥ 0.8 | 수익/MDD |
| **리스크** | MDD | ≤ 25% | ≤ 20% | ≤ 15% | 심리적 한계 |
| | 연속손실 | ≤ 8 | ≤ 6 | ≤ 5 | 전략 신뢰도 |
| | 변동성 | ≤ 70% | ≤ 60% | ≤ 50% | 연율 기준 |
| **통계** | 최소 거래 | ≥ 10 | ≥ 15 | ≥ 20 | 유의성 |
| **품질** | 평균손익비 | ≥ 1.0 | ≥ 1.2 | ≥ 1.3 | 개별 거래 |
| | 보유시간 | ≤ 336h | ≤ 240h | ≤ 168h | 자본효율 |

> 📚 **상세 가이드**: [BACKTESTING_GUIDE.md](./BACKTESTING_GUIDE.md) 참조
...
상세 흐름은 다음 다이어그램 참조:
- [Scheduler Main Flow](../diagrams/scheduler-main-flow.mmd) - scheduler_main.py 전체 흐름
- [Trading Job Flow](../diagrams/trading-job-flow.mmd) - trading_job() 상세 흐름
- [Position Management Flow](../diagrams/position-management-flow.mmd) - position_management_job() 상세 흐름
...
APScheduler (듀얼 타임프레임)
    ...
    ├─ [매 1시간] trading_job()
    │       ↓
    │   1. Lock 획득 (trading_cycle, timeout=600초)
    │       ├─ 실패 시 → 즉시 종료 (다른 작업 실행 중)
    │       └─ 성공 시 → 계속
    │       ↓
    │   2. Container 초기화 + TradingOrchestrator 획득
    │       ↓
    │   3. 📱 Telegram 알림 #1: 사이클 시작
    │       ↓
    │   4. 시장 데이터 수집 (로깅용)
    │       ↓
    │   5. TradingOrchestrator.execute_trading_cycle(enable_scanning=True)
    │       ↓
    │       5.1 Idempotency 체크 (fail-close 정책)
    │           ├─ 중복 → 스킵 (정상 종료)
    │           ├─ 체크 실패 → 거래 차단 (안전 우선)
    │           └─ 통과 → 계속
    │       ↓
    │       5.2 HybridTradingPipeline 실행 (10분 타임아웃)
    │           ├─ Stage 1: HybridRiskCheckStage (포지션 확인 + 멀티코인 스캔)
    │           │     ├─ CoinSelector.scan_liquidity() → 상위 10개
    │           │     ├─ MultiBacktest.run_multi_backtest() → 통과 5개
    │           │     └─ 📱 Telegram 알림 #2: 스캔 결과 + 백테스팅
    │           ├─ Stage 2: DataCollectionStage (시장 데이터)
    │           ├─ Stage 3: AnalysisStage (기술적 분석 + AI 분석)
    │           └─ Stage 4: ExecutionStage (거래 실행)
    │       ↓
    │       5.3 Idempotency 키 마킹 (성공 시)
    │       ↓
    │   6. 결과 처리
    │       ├─ [성공] →
    │       │     ├─ Prometheus 메트릭 기록 (AI 판단, 거래)
    │       │     ├─ PostgreSQL 저장 (ai_decisions, trades)
    │       │     ├─ 포트폴리오 정보 수집
    │       │     ├─ 📱 Telegram 알림 #3: AI 의사결정 상세
    │       │     └─ 📱 Telegram 알림 #4: 포트폴리오 현황
    │       ├─ [스킵] → 로그만 기록 (정상 동작)
    │       └─ [실패] → 📱 Telegram 알림 #5: 에러
    │       ↓
    │   7. Lock 해제 (finally 블록 - 반드시 실행)
    │
    └─ [매 15분] position_management_job()
            ↓
        1. Lock 획득 (trading_cycle 공유, timeout=60초)
            ├─ 실패 시 → 즉시 종료 (trading_job 실행 중)
            └─ 성공 시 → 계속
            ↓
        2. Container 초기화 + TradingOrchestrator 획득
            ↓
        3. TradingOrchestrator.execute_position_management()
            ↓
        규칙 기반 손절/익절 체크 (AI 없음, 빠른 처리)
            ↓
        4. 청산 발생 시 → 📱 Telegram 알림 (TODO)
            ↓
...
| 작업 | 주기 | 설명 | 타임아웃 | 안정성 |
|------|------|------|----------|--------|
| `trading_job` | 매시 01분 | 멀티코인 스캔 + AI 분석 + 진입 탐색 | 10분 | Lock(600초) + Idempotency(fail-close) |
| `position_management_job` | :01/:16/:31/:46 | 보유 포지션 손절/익절 관리 | - | Lock(60초, trading_cycle 공유) |
| `portfolio_snapshot_job` | 매시 01분 | 포트폴리오 스냅샷 DB 저장 | - | - |
| `daily_report_job` | 매일 09:00 | 일일 리포트 텔레그램 발송 | - | - |

**즉시 실행 옵션** (개발/테스트용):
- 환경변수: `SCHEDULER_RUN_IMMEDIATELY=true`
- 동작: 스케줄러 시작 2초 후 `trading_job` 즉시 실행
- 프로덕션: `SCHEDULER_RUN_IMMEDIATELY=false` (기본값, CronTrigger 스케줄 따름)

**스케줄러 안정성 메커니즘**:

1. **CronTrigger**
   - 캔들 마감 시점 정렬 (IntervalTrigger 대체)
   - 1분 버퍼: 캔들 데이터 안정화 대기 (매시 01분 실행)

2. **PostgreSQL Advisory Lock** (`LockPort`)
   - 작업 간 상호 배제 (동시 실행 방지)
   - Lock ID: `trading_cycle` (trading_job과 position_management_job 공유)
   - Timeout: trading_job=600초, position_management_job=60초
   - 획득 실패 시: 즉시 종료 (다른 작업 실행 중)
   - 해제: finally 블록에서 반드시 실행

3. **Idempotency Key** (`IdempotencyPort`, fail-close 정책)
   - 포맷: `{ticker}-{timeframe}-{candle_ts}-{action}`
   - 예시: `KRW-BTC-1h-2026010409-trading_cycle`
   - **Fail-Close 정책**: 체크 실패 시 거래 차단 (안전 우선)
   - TTL: 24시간 (동일 캔들 재실행 방지)
   - 중복 감지 시: status='skipped' (정상 동작, 성공 메트릭 기록)

4. **타임아웃 처리**
   - `trading_job`: 10분 타임아웃 (`asyncio.wait_for`)
   - 초과 시: TimeoutError 발생 → 에러 알림 → Lock 해제

5. **Graceful Shutdown**
   - SIGINT/SIGTERM 시그널 처리 (Ctrl+C)
   - 봇 상태 업데이트 → Telegram 종료 알림 → 스케줄러 정지

**Telegram 알림 흐름** (trading_job 기준):

| 순서 | 함수 | 시점 | 내용 |
|------|------|------|------|
| ① | `notify_cycle_start()` | 스캔 시작 전 | "멀티코인 스캐닝 시작" |
| ② | `notify_scan_result()` | 백테스팅 완료 후 (콜백) | 유동성 스캔 결과 + 백테스팅 통과 코인 + 선택된 코인 |
| ③ | `notify_ai_decision()` | AI 분석 완료 후 | AI 판단 (buy/sell/hold) + 신뢰도 + 근거 + 실행 시간 |
| ④ | `notify_portfolio_status()` | 거래 실행 후 | KRW 잔고 + 암호화폐 잔고 + 총 자산 + 거래 결과 |
| ⑤ | `notify_error()` | 에러 발생 시 | 에러 타입 + 메시지 + 컨텍스트 |
...
### v4.5.2 (2026-01-04) - 스케줄러 문서 업데이트 및 시퀀스 다이어그램 추가 📚

**주요 변경사항**:

1. **스케줄러 섹션 전체 재작성**
   - 실제 코드 구현과 일치하도록 문서 업데이트
   - `TradingService`, `AIService` → `Container` + `TradingOrchestrator` 반영
   - Idempotency fail-close 정책 상세 설명 추가
   - 타임아웃 처리 (10분) 명시
   - 즉시 실행 옵션 (`SCHEDULER_RUN_IMMEDIATELY`) 문서화

2. **시퀀스 다이어그램 추가** (Mermaid)
   - `scheduler-main-flow.mmd`: scheduler_main.py 전체 흐름
   - `trading-job-flow.mmd`: trading_job() 상세 흐름 (Lock → Idempotency → Pipeline → 알림 → Lock 해제)
   - `position-management-flow.mmd`: position_management_job() 상세 흐름
   - ARCHITECTURE.md에 다이어그램 링크 추가

3. **Telegram 알림 흐름 명확화**
   - 5단계 알림 순서 및 시점 표로 정리
   - ① 사이클 시작 → ② 스캔 결과 → ③ AI 판단 → ④ 포트폴리오 → ⑤ 에러

4. **안정성 메커니즘 상세 설명**
   - Lock 타임아웃: trading_job=600초, position_management_job=60초
   - Lock 공유: `trading_cycle` (두 작업 간 상호 배제)
   - Idempotency TTL: 24시간
   - Graceful Shutdown 프로세스 명시

5. **Full Path 시나리오 테스트 추가**
   - `tests/scenarios/test_scheduler_full_path.py`: 11개 테스트
   - `tests/scenarios/test_trading_job_scenarios.py`: 7개 테스트
   - 정상 경로, 에러 경로, Edge Case 모두 커버

**테스트**: 기존 테스트 유지, 시나리오 테스트 18개 추가

---

### v4.6.0 (2026-01-04) - 2-Gate 파이프라인 완전 통합 🆕

**주요 변경사항**:

1. **2-Gate 파이프라인 CoinSelector 통합**
   - `CoinSelector._apply_trading_pass()`: AI 분석 후 Trading Pass 검증 (4-1단계)
   - `CoinCandidate.is_ready_for_entry`: Trading Pass 통과 필수 조건 추가
   - `CoinCandidate.trading_pass_passed`, `expectancy_R` 필드 추가

2. **Expectancy Filter 필수화**
   - `evaluate_trading_pass()`에 Expectancy 검증 통합
   - 기대값 음수 전략은 Trading Pass 자동 실패
   - 결과에 기대값 정보 포함 (`net_expectancy`, `min_r_required`)

3. **Config 통합**
   - `MultiBacktestConfig`: Research Pass 기준으로 변경 (설정 중복 제거)
   - `from_research_config()` 팩토리 메서드 추가
   - 단일 소스 원칙 적용

4. **테스트 업데이트**
   - `test_two_gate_pipeline.py`: 2단 게이트 시나리오 테스트 (14개)
   - `test_expectancy_filter.py`: 기대값 계약 테스트 (24개)
   - 기존 테스트 Research Pass 기준으로 업데이트
...
**현재 버전**: 4.5.2
**마지막 업데이트**: 2026-01-04
**아키텍처**: Clean Architecture + Scheduler Stability + Multi-Coin Scanning + Dual-Timeframe Pipeline + 2-Gate Backtesting
**상태**: 프로덕션 준비 완료 ✅
**문서 상태**: ✨ 스케줄러 전체 흐름 및 시퀀스 다이어그램 업데이트 완료 (2026-01-04)
